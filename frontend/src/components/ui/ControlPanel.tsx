...
export default function ControlPanel({
  ...
  return (
    <div className="fixed bottom-4 left-1/2 z-30 w-[min(960px,95vw)] -translate-x-1/2">
      <div className="rounded-3xl border border-cyan-300/30 bg-slate-950/66 px-3 py-3 backdrop-blur-2xl shadow-[0_16px_80px_rgba(2,6,23,0.65)] md:px-4 md:py-4">
          ...
          <form className="flex min-w-0 flex-1 items-center gap-2" onSubmit={handleSubmit}>
            <div className="relative flex-1">
              ...
              <input
                ...
                className="h-12 w-full rounded-2xl border border-cyan-300/25 bg-slate-900/62 pl-9 pr-4 text-sm text-slate-100 outline-none transition placeholder:text-blue-100/35 focus:border-cyan-200/70 focus:bg-slate-900/82"
              />
            </div>

            <button
              type="submit"
              className="flex h-12 items-center gap-2 rounded-2xl border border-cyan-200/35 bg-cyan-400/14 px-4 text-xs font-semibold uppercase tracking-[0.16em] text-cyan-100 transition hover:bg-cyan-300/22"
            >
